...
UIPlayerParam::UIPlayerParam(std::string name,Vector3D pos){
...
void UIPlayerParam::Update(){
	...
	for(int i = 1; i < 4; i+=2){
		_staminaGauge[i].pos.x = _staminaGauge[i-1].pos.x + staminaGauge*2;
		_shootGauge[i].pos.x = _shootGauge[i-1].pos.x + shootGauge*2;
	}
};
...
void UIPlayerParam::Draw(){
	...
	DrawPolygonIndexed2D(_staminaGauge.data(), 4,index,2, DX_PRIMTYPE_TRIANGLESTRIP, DX_DRAWMODE_BILINEAR);
	DrawPolygonIndexed2D(_shootGauge.data(), 4, index, 2, DX_PRIMTYPE_TRIANGLESTRIP, DX_DRAWMODE_BILINEAR);
};
